...
//
// Created by cristi on 10/5/22.
//

#ifndef CS_LABS_BLOCK_H
#define CS_LABS_BLOCK_H
...
BlowfishBlockCypher::BlowfishBlockCypher(string key) : key(std::move(key)) {

    string key1;

    for (char i: this->key) {
        key1 += bitset<8>(i).to_string();
    }
    ...
    for (auto &i: P) {
        ...
        istringstream ss(i);
        unsigned long long int Pdec;
        ss >> hex >> Pdec;

        i = bitset<32>((bitset<32>(key1.substr(j, 32)).to_ulong() xor Pdec)).to_string();
...
string BlowfishBlockCypher::encryptMessage(const string &message) {

    string encryptedMessage;
    string message1;

    for (char i: message) {
        message1 += bitset<8>(i).to_string();
    }

    for (int i = 0; i < message1.length(); i += 64) {
        string l_side = message1.substr(i, 32), r_side = message1.substr(i + 32, 32);
...
string BlowfishBlockCypher::decryptMessage(const string &encryptedMessage) {

    string decryptedMessage;

    for (int i = 0; i < encryptedMessage.length(); i += 64) {
        string l_side = encryptedMessage.substr(i, 32), r_side = encryptedMessage.substr(i + 32, 32);
        for (int j = 17; j > 1; --j) {
            string n_r_side;
            n_r_side = bitset<32>(bitset<32>(l_side) xor bitset<32>(P[j])).to_string();
        ...
        decryptedMessage += bitset<32>(bitset<32>(r_side) xor bitset<32>(P[0])).to_string();
        decryptedMessage += bitset<32>(bitset<32>(l_side) xor bitset<32>(P[1])).to_string();
    ...
    for (int i = 0; i < decryptedMessage.length(); i += 8) {
        decryptedMessage1 += char(bitset<8>(decryptedMessage.substr(i, 8)).to_ulong());
    }

    return decryptedMessage1;
}

string BlowfishBlockCypher::F(const string &bits) {
    unsigned long long int cols[4];
    for (int i = 0; i < bits.length(); i += 8) {
        cols[i / 8] = bitset<8>(bits.substr(i, 8)).to_ulong();
    }

    unsigned long long int s_box[4];

    for (int i = 0; i < 4; ++i) {

        istringstream ss(S[i][cols[i]]);
        unsigned long long int Sdec;
        ss >> hex >> Sdec;
    ...
    ans = (s_box[0] + s_box[1]) % 4294967296;
    ans ^= s_box[2];
    ans = (ans + s_box[3]) % 4294967296;

    return bitset<32>(ans).to_string();
}


#endif //CS_LABS_BLOCK_H
